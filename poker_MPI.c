...
int main(int argc,char** argv){
	...
	//Compute how many trials each process is responsible for using the total, and the number of processes
	h = cnt/comm_sz;
	...
	MPI_Reduce(&straightFlushes,&total_count,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
	
	if(my_rank == 0){
		percent=(float)total_count/(float)cnt*100.0;
		printf("We found %d straight flushes out of %d hands or %f percent.\n",total_count,cnt,percent);
	}
	if(my_rank == 0){
		t2 = MPI_Wtime();
		printf("it took %1.6f seconds\n", t2-t1);
	}
